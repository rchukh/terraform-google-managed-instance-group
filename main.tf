...
resource "google_compute_instance_group_manager" "default" {
  ...
  auto_healing_policies {
    health_check      = var.http_health_check ? concat(google_compute_health_check.mig-health-check.*.self_link, list(""))[0] : ""
...
resource "google_compute_region_instance_group_manager" "default" {
  ...
  target_size = var.autoscaling ? null : var.size
...
data "google_compute_instance_group" "zonal" {
  ...
  name = split("|", "${local.dependency_id}|${element(concat(google_compute_instance_group_manager.default.*.name, list("unused")), 0)}")[1]
}
